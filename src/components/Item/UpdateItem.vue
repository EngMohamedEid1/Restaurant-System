  ...
  methods: {
    ...
    async getItemInfo(userId, locId, itemId) {
      ...
      if (result.status == 200 && result.data.length > 0) {
        ...
        this.Description = itemDetails[0].Description;
        this.pickedCategory = itemDetails[0].catId;
        this.itemName = itemDetails[0].name;
        this.itemPrice = itemDetails[0].price;
        this.itemQty = itemDetails[0].Qty;
        // this.locAddress = itemDetails[0].address;
        // this.locPhone = itemDetails[0].phone;
      }
    },
    async updateItem() {
      this.v$.$validate();
